...
import { B as SafeLocalStorageKeys, J as ConstantsUtil, S as BlockchainApiController, n as ChainController, q as NetworkUtil, t as ApiController, u as getActiveCaipNetwork, z as SafeLocalStorage } from "./ApiController-B1_7t7zN.js";
...
var ReownAuthenticationMessenger = class {
	...
	async createMessage(input) {
		...
		return Object.assign(params, { toString: () => this.stringify(params) });
	}
	stringify(params) {
...
var ReownAuthentication = class {
	constructor(params = {}) {
		...
		this.messenger = new ReownAuthenticationMessenger({ getNonce: this.getNonce.bind(this) });
	}
	async createMessage(input) {
	...
	async getSessions(chainId, address) {
		try {
			...
			if (!isSameAddress || !isSameNetwork) return [];
			const session = {
				data: {
	...
	async setSessions(sessions) {
		if (sessions.length === 0) this.clearStorageTokens();
		else {
	...
	async getSessionAccount() {
		...
		return this.request({
			...
			key: "me",
			body: void 0,
			query: { includeAppKitAccount: true },
			headers: ["auth"]
		});
	}
	...
	async request({ method, key, query, body, headers }) {
		...
		if (query) Object.entries(query).forEach(([queryKey, queryValue]) => url.searchParams.set(queryKey, String(queryValue)));
			...
			headers: Array.isArray(headers) ? headers.reduce((acc, header) => {
				switch (header) {
					...
					case "auth":
						...
						break;
					case "otp":
						if (this.otpUuid) acc["x-otp"] = this.otpUuid;
						break;
					default: break;
				}
				return acc;
	...
	getWalletInfo() {
		...
		if ("social" in walletInfo && "identifier" in walletInfo) return {
			type: "social",
	...
	setAppKitAccountUser(session) {
		const { email } = session;
		if (email) Object.values(ConstantsUtil.CHAIN).forEach((chainNamespace) => {
